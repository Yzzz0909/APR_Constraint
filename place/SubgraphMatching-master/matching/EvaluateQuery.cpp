    ...
    while (true) {
        while (idx[cur_depth] < idx_count[cur_depth]) {
            ...
            if (cur_depth == max_depth - 1) {
                embedding_cnt += 1;
                std::vector<ui> current_embedding_map(max_depth);
                for (ui i = 0; i < max_depth; ++i) {
                    current_embedding_map[i] = embedding[i];
                }
                //results.push_back(current_embedding_map);
...
EvaluateQuery::LFTJ(const Graph *data_graph, const Graph *query_graph, Edges ***edge_matrix, ui **candidates,
                    ...
                    std::vector<std::vector<ui>>& results) {
    ...
    while (true) {
        while (idx[cur_depth] < idx_count[cur_depth]) {
            ...
            if (cur_depth == max_depth - 1) {
                embedding_cnt += 1;
                 std::vector<ui> current_embedding_map(query_graph->getVerticesCount());
                 for (ui i = 0; i < max_depth; ++i) {
                    current_embedding_map[order[i]] = embedding[order[i]]; // <--- 使用原始 ID 作为目标索引
                }
                printf("C++ DEBUG: LFTJ Saved Embedding. Q:%u -> D:%u\n", order[0], embedding[order[0]]);
                results.push_back(current_embedding_map); 
                // std::vector<ui> current_embedding_map(max_depth);
                // for (ui i = 0; i < max_depth; ++i) {
                //     current_embedding_map[i] = embedding[i];
                // }
                // printf("C++ DEBUG: LFTJ Mapping Found. Q:%u -> D:%u\n", order[0], embedding[order[0]]);
                // results.push_back(current_embedding_map);
                visited_vertices[v] = false;
