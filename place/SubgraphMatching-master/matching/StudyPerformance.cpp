...
int main(int argc, char** argv) {
    ...
    std::string input_result_file_path = command.getResultFilePath();
    std::vector<std::vector<ui>> all_embeddings;
    /**
    ...
    std::cout << "\tResult File Path: " << input_result_file_path << std::endl;
    std::cout << "--------------------------------------------------------------------" << std::endl;
    ...
    if (input_engine_type == "EXPLORE") {
        ...
        embedding_count = EvaluateQuery::LFTJ(data_graph, query_graph, edge_matrix, candidates, candidates_count,
                                              matching_order, output_limit, call_count, all_embeddings);
...
#endif
     printf("DEBUG: all_embeddings size is %zu\n", all_embeddings.size());
     if (!input_result_file_path.empty() && !all_embeddings.empty()) {
        std::ofstream outfile(input_result_file_path, std::ios_base::out); // 使用覆盖模式
        if (outfile.is_open()) {
            // 写入格式：模板节点ID 数据节点ID
            for (const auto& emb : all_embeddings) {
                for (ui i = 0; i < emb.size(); ++i) {
                    // i 是模板节点ID (Template ID), emb[i] 是数据节点ID (Target ID)
                    outfile << i << " " << emb[i] << "\n";
                }
            }
             printf("DEBUG: Successfully wrote %zu embeddings to file.\n", all_embeddings.size());
            outfile.close();
        } else {
            // 写入失败时，打印错误信息
            std::cerr << "Error: Could not open result file for writing: " << input_result_file_path << std::endl;
        }
    }
