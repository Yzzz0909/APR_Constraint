...
# from GANA.src.BasicElement import BasicElement
class BasicElement:
    def __init__(self,line):
        line=line.replace(')',"").replace('(',"")
        self.line = line
    # def total_size(self, values): # 存在问题：仅识别p的量级的
    #     total = 1
    #     for value in values:
    #         if '='in value:
    ...
    #         #     total = total*self.is_float(value.replace('K',''))
    #         else:
    ...
    def total_size(self, values):
        ...
        for value in values:
            ...
            if val_str.endswith('meg'):
                total = total * self.is_float(val_str.replace('meg', '')) * 1.0e6
            elif val_str.endswith('t'):
                total = total * self.is_float(val_str.replace('t', '')) * 1.0e12
            elif val_str.endswith('g'):
                total = total * self.is_float(val_str.replace('g', '')) * 1.0e9
            elif val_str.endswith('k'):
                total = total * self.is_float(val_str.replace('k', '')) * 1.0e3
            elif val_str.endswith('m'):
                total = total * self.is_float(val_str.replace('m', '')) * 1.0e-3
            elif val_str.endswith('u'):
                total = total * self.is_float(val_str.replace('u', '')) * 1.0e-6
            elif val_str.endswith('n'):
                total = total * self.is_float(val_str.replace('n', '')) * 1.0e-9
            elif val_str.endswith('p'):
                total = total * self.is_float(val_str.replace('p', '')) * 1.0e-12
            elif val_str.endswith('f'):
                total = total * self.is_float(val_str.replace('f', '')) * 1.0e-15
            else:
                total = total * self.is_float(val_str)
    ...
    def Resistor(self):
        #Res: c7 net7 vinp c1
        [resistance, plus, minus, reference]=self.line.strip().split()[0:4]
        value = self.line.strip().split()[4:]

        edges = [plus,minus]
        edge_weight = [8,8]
        return {"inst": resistance, "inst_type": "res", "reference": reference, "ports": edges, "edge_weight": edge_weight, "values": self.total_size(value)}
    ...
    def Transistor(self):
        ...
        if 'n' in inst_type.lower():
            ...
            inst_type = "pmos"
        return {"inst": inst, "inst_type": inst_type, "reference" : reference, "ports": edges, "edge_weight": edge_weight, "values": self.total_size(value)}
...
class spiceParser2: # for empyrean spice netlist
    ...
    def sp_parser(self):
        if not os.path.isfile(self.netlist):
            ...
            fp = open(self.netlist, "r")
            line = fp.readline()
            while "TopDesEnd" not in line and not line.strip().lower() in ('.end', 'end'):
    ...
    def _parse_inst(self, line):
        ...
        if not line.strip():
            return device
        elif line.strip().lower().startswith('m') or line.strip().lower().startswith('n') or line.strip().lower().startswith('p') or line.strip().lower().startswith('t') or line.strip().lower().startswith('xm'):
    ...
    def _create_circuit_graph(self, all_nodes, inout_ports):
        logging.info(
            "Creating graph with Total no of devices " + str(len(all_nodes)))
        circuit_graph = nx.Graph()
        for node in all_nodes:
            if "hier_nodes" in node.keys():
                subgraph = self._create_circuit_graph(node["hier_nodes"],self.subckts[node["inst_type"]]["ports"])
            else:
                subgraph = None
            circuit_graph.add_node(node["inst"],
                                   inst_type=node["inst_type"],
